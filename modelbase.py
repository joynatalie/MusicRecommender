...
def generate_playlist_feature(complete_feature_set, playlist_df, weight_factor):
    """
    Summarize a user's playlist into a single vector Parameters: complete_feature_set (pandas dataframe): Dataframe
    which includes all the features for the spotify songs playlist_df (pandas dataframe): playlist dataframe
    weight_factor (float): float value that represents the recency bias. The larger the recency bias,
    the most priority recent songs get. Value should be close to 1.
    ...
    # complete_feature_set_playlist = complete_feature_set_playlist.merge(playlist_df[['id','date_added']],
    # on = 'id', how = 'inner')
...
def recommend_from_playlist(songDF=songDF, complete_feature_set=complete_feature_set, playlistDF_test=playlistDF_test):
    ...
    complete_feature_set_playlist_vector_EDM, complete_feature_set_nonplaylist_EDM = generate_playlist_feature(
        complete_feature_set, playlistDF_test, 1.09)

    # Generate recommendation
    edm_top40 = generate_playlist_recos(songDF, complete_feature_set_playlist_vector_EDM,
